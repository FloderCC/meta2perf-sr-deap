"""
Script to generate an initial population for symbolic regression in DEAP.
The representation is a fixed-length prefix vector with a configurable maximum depth. The default max_depth is 35, but most generated equations will be much shallower because projected leaf nodes terminate early.
"""
...
class SrVecParser:
    ...
    The chromosome is a fixed-length list of prefix tokens. Each token uses 5 genes:
        ...
        [node_type, var_code, unary_code, binary_code, const_value]
    ...
    * max_tokens bounds the prefix stream length and keeps the chromosome fixed-size,
    ...
    def __init__(
        ...
        probs = [p_var, p_const, p_unary, p_binary]
        total = sum(probs)
        if total <= 0:
            raise ValueError('The sum of node probabilities must be > 0.')
        self.node_probs = [p / total for p in probs]
        self._leaf_probs = [self.node_probs[0], self.node_probs[1]]
    ...
    def encode_equation(self, equation_str: str) -> list:
        ...
        def build_tokens(node, depth):
            ...
            if const_value is not None:
                return [[self.NODE_CONST, 0, 0, 0, float(const_value)]]
            ...
            if isinstance(node, ast.Call):
                ...
                if op_name in unary_to_idx:
                    ...
                    return [[self.NODE_UNARY, 0, unary_to_idx[op_name], 0, 0.0]] + child_tokens
                ...
                if op_name in binary_to_idx:
                    ...
                    return [[self.NODE_BINARY, 0, 0, binary_to_idx[op_name], 0.0]] + left_tokens + right_tokens
    ...
    def _project_node_type(
        ...
        *,
        ...
        if depth < self.max_depth - 1 and self.unary_operators:
    ...
    def _decode_expr_slots(
        ...
        raw_type, var_code, unary_code, binary_code, const_value = tokens[pos]
        next_pos = pos + 1
    ...
    def validate_vector_structure(self, encoded_genes: list) -> dict:
        ...
        for i in range(0, len(encoded_genes), self.GENES_PER_TOKEN):
            ...
            unary_code = self._sanitize_index(encoded_genes[i + 2], len(self.unary_operators))
            binary_code = self._sanitize_index(encoded_genes[i + 3], len(self.binary_operators))
            const_value = float(encoded_genes[i + 4])
            tokens.append((node_type, var_code, unary_code, binary_code, const_value))
    ...
    def random_individual(self, const_min: float = -1.0, const_max: float = 5.0) -> list:
        ...
        def fill(pos: int, depth: int, open_slots: int) -> Tuple[int, int]:
            ...
            tokens[pos] = [node_type, var_code, unary_code, binary_code, const_value]
            next_pos = pos + 1
...
def create_pop_for_deap(
    ...
    def generate_random_unique_population():
        ...
        while len(population) < pop_size and attempts < total_attempts:
            ...
            key = canonical_expr if unique_individuals else tuple(individual)
            if key in seen:
                continue
            seen.add(key)
    ...
    if method == 'random':
        population = generate_random_unique_population() if unique_individuals else [
            parser.random_individual(const_min=const_min, const_max=const_max) for _ in range(pop_size)
        ]
        ...
        if oblesa_population is not None:
            encoded_population = (
                ...
                if unique_individuals
                    ...
                    parser.random_individual(const_min=const_min, const_max=const_max)
                    for _ in range(pop_size)
                ]
            )

            n_keep = len(encoded_population) - len(oblesa_population)
            if n_keep > 0:
                scored_population = [
                ...
                encoded_population = [
                    ind for _, ind in heapq.nlargest(n_keep, scored_population, key=lambda x: x[0])
                ]
            else:
                encoded_population = []
            ...
            encoded_population = None

        print(f"The optimization space for oblesa has {len(bounds)} dimensions and the bounds are: {bounds[0:5]}")
